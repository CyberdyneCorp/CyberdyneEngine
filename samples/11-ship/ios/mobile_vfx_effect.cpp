...
[[nodiscard]] Status build_spawn(Allocator& allocator, Emitter& emitter) noexcept {
    ...
    if (!stage.ok()) {
        return fail(ErrorCode::Internal, "iOS mobile VFX: the spawn graph did not author");
    }
...
[[nodiscard]] Status build_initialise(Allocator& allocator, Emitter& emitter) noexcept {
    ...
    if (!stage.ok()) {
        return fail(ErrorCode::Internal, "iOS mobile VFX: the initialise graph did not author");
    }
...
[[nodiscard]] Status build_update(Allocator& allocator, Emitter& emitter) noexcept {
    ...
    if (!stage.ok()) {
        return fail(ErrorCode::Internal, "iOS mobile VFX: the update graph did not author");
    }
