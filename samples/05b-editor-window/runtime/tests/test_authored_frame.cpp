...
void verify_mesh_and_transforms(AuthoredFrame& frame, const ser::AuthoringSchema& schema,
...
void verify_lighting(AuthoredFrame& frame, const ser::AuthoringSchema& schema,
                     const first_light::Camera& view) {
    ser::World sphere(allocator());
    ser::World lit(allocator());
    CY_REQUIRE(ser::read_world(kSphere, "worlds/test.cyworld", sphere).has_value());
    ...
    CY_REQUIRE(ser::read_world(directional_text, "worlds/test.cyworld", directional).has_value());
...
void verify_shadows(AuthoredFrame& frame, const ser::AuthoringSchema& schema,
                    const first_light::Camera& view) {
    ...
    CY_REQUIRE(ser::read_world(receiver_off_text, "worlds/test.cyworld", receiver_off).has_value());
    ...
    const usize shadow_rotation = shadow_rotated_text.rfind("field 1 0 0.9238795 0.3826834 0\n");
    CY_REQUIRE(shadow_rotation != std::string::npos);
    shadow_rotated_text.replace(shadow_rotation, sizeof("field 1 0 0.9238795 0.3826834 0\n") - 1,
                                "field 1 -0.2705981 0.6532815 0.2705981 0.6532815\n");
    ser::World shadow_rotated(allocator());
    CY_REQUIRE(
        ser::read_world(shadow_rotated_text, "worlds/test.cyworld", shadow_rotated).has_value());
...
void verify_material_preview(AuthoredFrame& frame, const first_light::Camera& view) {
    ...
    CY_REQUIRE(assets::fs::read_whole((std::string(CY_TEST_PROJECT) + "/" + reference).c_str(),
                                      graph_bytes));
    std::string graph_source(reinterpret_cast<const char*>(graph_bytes.data()), graph_bytes.size());
...
CY_TEST_CASE("authored native frame renders a mesh and publishes its transformed bounds") {
...

    {
        ...
        verify_mesh_and_transforms(frame, schema, view);
        verify_lighting(frame, schema, view);
        verify_shadows(frame, schema, view);
        verify_material_preview(frame, view);
    }
