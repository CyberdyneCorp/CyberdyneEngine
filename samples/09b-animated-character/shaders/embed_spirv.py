...
Profile and Shipping and can be turned off anywhere — so character.slang is compiled once, by hand,
and embedded. This script is how that is done reproducibly rather than by pasting numbers.

It is the same script as src/rendering/skinning/shaders/embed_spirv.py and
tests/render/shaders/embed_spirv.py, kept separate for the reason those two are kept separate from
each other: they write different namespaces, and a shared one would need a parameter nobody
...
HEADER = """#pragma once
// Compiled SPIR-V for the animated character's shading. GENERATED — do not edit by hand.
//
// Produced by samples/09b-animated-character/shaders/embed_spirv.py from character.slang; that
...
def main(argv: list[str]) -> int:
    ...
    for pair in argv[2:]:
        ...
        for start in range(0, len(words), 6):
            ...
            body.append(f"    {row},\n")
        body.append("};\n\n")
    body.append("}  // namespace cy::sample::character\n")
