...
//   * OFF, AND UNDER A CLEAR SKY, THE FRAME IS THE SAME FRAME. Every pixel, bit for bit: the
//     frame with the field disabled and the frame with the field enabled over a sky with no cloud
//     in it. "Cloud shadows cost nothing where there is no cloud" is what keeps every committed
//     capture of this world valid.
...
cy::Status WorldPass::prepare(const ShadowField& widest) noexcept {
    ...
    vertex.spirv = cy::Span<const u32>(cy::sample::world::kWorldVertexSpirv,
                                       sizeof(cy::sample::world::kWorldVertexSpirv) / sizeof(u32));
    auto vertex_module = device_.create_shader_module(vertex);
    if (!vertex_module) {
        return cy::make_unexpected(vertex_module.error());
    }
    ...
    fragment.spirv =
        cy::Span<const u32>(cy::sample::world::kWorldFragmentSpirv,
                            sizeof(cy::sample::world::kWorldFragmentSpirv) / sizeof(u32));
    auto fragment_module = device_.create_shader_module(fragment);
    if (!fragment_module) {
        return cy::make_unexpected(fragment_module.error());
    }
    ...
    pipeline.vertex_bindings = cy::Span<const rhi::VertexBinding>(vertex_bindings, 2);
    pipeline.vertex_attributes = cy::Span<const rhi::VertexAttribute>(attributes, 3);
    pipeline.color_attachments = cy::Span<const rhi::ColorAttachmentState>(&color, 1);
    ...
    auto created = device_.create_graphics_pipeline(pipeline);
    if (!created) {
...
CY_TEST_CASE(
    "world cloud shadows: ground under a cloud is darker, and ground beside it unchanged") {
    Scene scene;
...
CY_TEST_CASE("world cloud shadows: off, or under a clear sky, the frame is the same frame") {
    ...
    // Four ways of saying "no cloud shadow": the placeholder the stage binds before its first
    // frame, the cloudy field bound and switched off, the clear field bound and switched off, and
    // the clear field switched ON. The last is the claim that matters — a clear sky costs the
    // picture nothing even when the field is being read — and the first three are the frame with
    // cloud shadows off, which is the frame this world drew before they existed.
    ...
    const u32 off_changed = differing(placeholder, cloudy_off);
    const u32 clear_changed = differing(placeholder, clear_off);
    const u32 clear_on_changed = differing(placeholder, clear_on);
    const u32 cloudy_changed = differing(placeholder, cloudy_on);
    CY_TEST_MESSAGE("texels differing from the frame without cloud shadows: cloudy off ",
                    off_changed, ", clear off ", clear_changed, ", clear on ", clear_on_changed,
                    ", cloudy on ", cloudy_changed);
