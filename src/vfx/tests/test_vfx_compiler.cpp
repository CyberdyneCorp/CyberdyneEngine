...
/// update kernel — which loads `velocity` because it runs over particles it did not initialise. So
/// a claim about the FUSED kernel has to be made about the fused kernel's own text, cut out between
/// its `stages Initialise Update` header comment and the next kernel's.
///
/// A unit that will not assemble, or one with no fused kernel in it, answers TRUE: this returns
/// "the round trip is there" for every case in which it could not show that it is not.
[[nodiscard]] bool fused_kernel_loads_an_attribute(const CompiledEmitter& emitter) noexcept {
    Array<char> unit(allocator());
    Array<ParameterDecl> none(allocator());
    Array<EventChannelDecl> no_channels(allocator());
    if (!assemble_translation_unit(emitter, none.span(), no_channels.span(), unit).has_value()) {
    ...
    const std::string_view text(unit.data(), unit.size());
    const usize header = text.find("stages Initialise Update");
    if (header == std::string_view::npos) {
        return true;
    }
    const usize next = text.find("// Generated by cy::vfx.", header);
    const std::string_view body =
        next == std::string_view::npos ? text.substr(header) : text.substr(header, next - header);
    return body.find("cyVfxLoad_") != std::string_view::npos;
}
...
CY_TEST_CASE("kernel fusion: initialise and update share one dispatch over the newly spawned") {
    ...
    // these two lines only the first was checked: the slot counts below were formatted to stderr
    ...
    CY_CHECK(source_contains(with->emitters()[0], "cyVfxLoad_velocity"));
    CY_CHECK_FALSE(fused_kernel_loads_an_attribute(with->emitters()[0]));
}
