#!/usr/bin/env python3
"""Turn the compiled hierarchical-depth reduction into the checked-in C++ header.

The culling pass must exist in a build with no shader compiler — `CY_SHADER_SLANG` is off in Profile
and Shipping, and a renderer whose occlusion cull only exists where Slang was built is a renderer with
two different frames. So hzb_reduce.slang is compiled once, by hand, and embedded; this script is how that
...
HEADER = """#pragma once
// Compiled SPIR-V for the hierarchical depth buffer's reduction. GENERATED — do not edit by hand.
//
// Produced by src/rendering/hzb/shaders/embed_spirv.py from hzb_reduce.slang; that file's
// header comment carries the exact slangc invocations. Checked in rather than compiled by the build
// because the pyramid must exist in a build with NO shader compiler at all — which is every
...
def main(argv: list[str]) -> int:
    ...
    for pair in argv[2:]:
        ...
        for start in range(0, len(words), 6):
            ...
            body.append(f"    {row},\n")
        body.append("};\n\n")
    body.append("}  // namespace cy::rendering::hzb\n")
