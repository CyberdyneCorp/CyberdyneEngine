#!/usr/bin/env python3
"""Turn the compiled culling dispatch into the checked-in C++ header.

The culling pass must exist in a build with no shader compiler — `CY_SHADER_SLANG` is off in Profile
and Shipping, and a renderer whose cull only exists where Slang was built is a renderer with two
different frames. So gpu_cull.slang is compiled once, by hand, and embedded; this script is how that
...
HEADER = """#pragma once
// Compiled SPIR-V for the GPU culling dispatch. GENERATED — do not edit by hand.
//
// Produced by src/rendering/gpu_culling/shaders/embed_spirv.py from gpu_cull.slang; that file's
// header comment carries the exact slangc invocations. Checked in rather than compiled by the build
// because the culling pass must exist in a build with NO shader compiler at all — which is every
...
def main(argv: list[str]) -> int:
    ...
    for pair in argv[2:]:
        ...
        for start in range(0, len(words), 6):
            ...
            body.append(f"    {row},\n")
        body.append("};\n\n")
    body.append("}  // namespace cy::rendering::gpu_culling\n")
