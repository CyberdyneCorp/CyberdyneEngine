...
is byte-identical to one with no pass attached at all, and that frame matches
`tests/references/ambient_occlusion_absent.png`, rendered by the frame shader as it was before
this stage (`render.ambient_occlusion`, case (d)). Regenerate the reference only from that older
shader: substitute main's `frame_spirv.h`, run the suite with `CY_RENDER_UPDATE_GOLDEN=1`, restore.
...
* **Temporal accumulation of the term.** The search's noise is a 4x4 pattern fixed in screen space,
  ...
  With temporal anti-aliasing on, the jitter moves the depth and the term follows it; the second
  (e) case measures the resolved frame over the occluded pixels the frame without the stage leaves
  still (measured 0.023 of an 8-bit step per channel and 0.039 of the term per frame, bounded at
  0.035 and 0.045; with the cascade removed the same frames measure 0.043 and 0.051). Reseeding the
  search's noise every frame measures 0.026 and 0.035: the cascade removes it, which is why no
  history is kept for this signal.
