...
//       same number of declared passes;
//   (e) four frames of a still view: the term moves by at most `kStableTolerance` between frames.
//       Rendered WITHOUT the scene's temporal anti-aliasing, whose jitter sequence moves the depth
//       a sub-pixel each frame even when pinned — the term then follows the jittered depth, and
//       that motion is the resolve's to average, not noise the search adds.
//
...
struct FrameCase {
    ...
    /// The scene's temporal anti-aliasing, on as the pipeline suites render it. (e) turns it off:
    /// with it on, the jitter sequence moves the depth a sub-pixel every frame, and the term
    /// follows the depth it is given.
    bool temporal = true;
};

void configure_case(rendering::assembly::AssemblyDescription& description,
                    rendering::pipeline::PipelineSetup& setup, void* user) noexcept {
    ...
    if (!frame->temporal) {
        description.post.temporal_antialiasing = false;
        setup.prepass_velocity = false;
    }
}
