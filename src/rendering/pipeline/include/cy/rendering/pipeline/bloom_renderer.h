...
// clamped linear sampler. It owns no texture: every level is a graph transient. It allocates one
// per-frame descriptor set per recorded step, because a set a command buffer has bound may not be
// updated.
//
...
// entry points compile to HLSL and DXIL; a D3D12 port needs that vertex fix, a DXIL copy of these
// modules, and set 2 mapped to its register space.
