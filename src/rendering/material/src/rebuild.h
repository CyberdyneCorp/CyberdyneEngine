#pragma once
// The rebuild: how every pass and every derivation is expressed. M7 task 6.1.
//
// design.md §1.3: "Every optimisation pass is expressed as a bottom-up REBUILD through the same
// builder. That is not a shortcut: it is what guarantees a pass's output is in the same canonical
// form as its input, so the pipeline can be iterated to a fixed point and the fixed point is
// reachable from either front-end."
//
// One template rather than two loops, because passes.cpp and lowering.cpp do the same walk and
// differ only in what they decide per node — and a second copy of "carry the flags and the origins
// across" is exactly where an attribution quietly stops arriving.
//
// A rewrite MAY return `kInvalidNode`, which means the node is dropped. Its consumers see
// `kInvalidNode` in their mapped operand list and decide for themselves: dropping a leaf closure
// out of a `ClosureAdd` is correct, and dropping the `ClosureAdd` itself deletes everything beneath
// it — the mistake that gave the spike's first far-field program zero closures (design.md §1.5).
