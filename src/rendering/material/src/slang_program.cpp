...
void write_accessors(Writer& writer, u32 texture_count, u32 field_count) noexcept {
    if (texture_count > 0) {
        writer.text(
            ...
            "///\n"
            "/// THE STAGE DECIDES, and until M11.c it did not: this emitted only the explicit\n"
            "/// form, so `cy_material_sample` always read level 0 and the cooked mip chain was\n"
            "/// UNREACHABLE. M11.c's spike measured it — a frame with eight of nine levels never\n"
            "/// uploaded is BYTE-IDENTICAL, 0.00%% of texels differ — so the importer spent a "
            "third\n"
            "/// more bytes on a chain no generated program could read, and every minified "
            "surface\n"
            "/// would alias. A pixel stage defines CY_MATERIAL_PIXEL_STAGE and gets the implicit\n"
            "/// form; the compute probe below does not and keeps the explicit one, because\n"
            "/// implicit derivatives exist only in a pixel stage.\n"
            "///\n"
            "/// THE EXPLICIT LEVEL, where it is still used, is a limitation rather than a "
            "choice:\n"
            "/// implicit\n"
            ...
            "#if defined(CY_MATERIAL_PIXEL_STAGE)\n"
            "float4 cy_material_sample(CyMaterialContext ctx, uint slot, float2 uv)\n"
            "{\n"
            "    return cyMaterialSampleTexture(ctx.params.textures[slot], uv);\n"
            "}\n"
            "#else\n"
            "float4 cy_material_sample(CyMaterialContext ctx, uint slot, float2 uv)\n"
            "{\n"
