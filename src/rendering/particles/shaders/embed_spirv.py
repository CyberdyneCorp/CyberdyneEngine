#!/usr/bin/env python3
"""Turn the particle renderer's compiled shader module into the checked-in C++ header.

`CY_SHADER_SLANG` is off by default, so an ordinary build has no Slang compiler to call. The frame's
Slang source — `src/rendering/shaders/cy/particle.slang` — is therefore compiled once, by hand, and embedded; this script is how that
...
// `CY_SHADER_SLANG` is off by default and in Profile and Shipping, and an effect that only draws on
// a machine with a shader toolchain is an effect nobody can run. `src/backends/shader/`'s SPIR-V
...
def main(argv: list[str]) -> int:
    ...
    for pair in argv[2:]:
        ...
        for start in range(0, len(words), 6):
            ...
            body.append(f"    {row},\n")
        body.append("};\n\n")
    body.append("}  // namespace cy::rendering::particles\n")
