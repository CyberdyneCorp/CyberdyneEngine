...
MetalSeedStatus metal_seed_status() noexcept {
    ...
    status.open_gaps = metal_open_gap_count();
    ...
    status.renders =
        status.compiled_with_metal
            ...
            : "nothing on this platform. The seed's deliverable here is `mapping.h` and the eight "
              "gaps it names, which are built and tested wherever the engine is — and, since "
              "M11.d settled the interface on Vulkan and the null backend, what each of them "
              "became.";
    return status;
}
