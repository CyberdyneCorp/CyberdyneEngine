...
def compile_one(compiler: str, includes: list[str], source: pathlib.Path) -> tuple[bool, str]:
    ...
    if is_msvc:
        # /nologo suppresses the banner; /std:c++20 is the standard; /EHs-c- and /GR- match the
        # engine's -fno-exceptions / -fno-rtti; /Zs is the syntax-only equivalent of -fsyntax-only.
