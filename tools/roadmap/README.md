...
| `quiet_host.py` | Whether a criterion that runs a timing-sensitive suite runs ALL of it inside `just test-quiet-host`: nothing after the wrapped command on its line but `\|\| exit <n>`, no named suite run bare, and `exclusive` declared. M11.c's eighth close found `m6:culling`'s second suite running after the wrapper had exited. `tools/quiet-host/README.md` has the rule and the suites it names. |
