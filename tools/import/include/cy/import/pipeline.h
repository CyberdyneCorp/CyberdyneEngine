...
/// The cached-bundle encoding: every sub-asset's kind, name, primary flag and payload, in the order
/// the importer produced them. M7 task 1.4 made these public.
///
/// It was file-local to `pipeline.cpp` for as long as the cook cache was the only thing that stored
/// an import. A build-graph node stores one too — its single declared output IS the bundle — and a
/// second encoding of the same result would be a second thing to keep in step. Deterministic by
/// construction: no timestamps, no paths, no iteration over a hash table.
///
/// Diagnostics are deliberately NOT in it. A warning is about the source and is regenerated by
/// re-running the importer; storing it would mean a cache hit reporting warnings for a state of the
/// file that may no longer exist.
[[nodiscard]] Status encode_import_bundle(const ImportResult& result, Array<u8>& out) noexcept;
[[nodiscard]] Status decode_import_bundle(Span<const u8> payload, ImportResult& out) noexcept;
