...
Defines **CyberAnimation**: skeletal animation, procedural rigging, and pose processing, designed
for tens of thousands of animated instances.

The structuring decisions follow the patterns established elsewhere in the engine. **Skeleton and
rig are separate assets** — the skeleton is the lean runtime deformation hierarchy, the rig is the
artist-facing control system that compiles to a program. **Graphs and rigs are compiled**, not
interpreted, and shared across every instance using them. **Pose evaluation is separated from
skinning**, with a **GPU pose world** — animation's analogue of the GPU scene — holding current and
previous bone matrices for skinning, motion vectors, and VFX attachment, without CPU readback.
**Animation LOD** reduces fidelity as well as rate, and **pose sharing** lets a crowd in the same
walk cycle evaluate a handful of poses rather than ten thousand.

**Motion matching** is core rather than an add-on, because hand-authored locomotion state machines
are where most animation authoring goes and where most visible quality problems remain.

One boundary is stated as a requirement: root motion drives the character controller, so it is
gameplay state. It SHALL be computed on a deterministic CPU path regardless of where the rest of
the pose is evaluated or at what LOD tier the instance runs — because deriving it from a
GPU-evaluated pose, or from a tier chosen by measured frame time, would silently break network
