# weather-and-wind Specification

## Purpose

Defines **CyberWeather**: the producer the field substrate was waiting for.

The governing rule is that **weather publishes state and touches nothing**. It does not iterate
materials setting wetness, walk foliage setting sway, or poke particle systems; it writes fields, and
consumers sample what they need at the fidelity they need. A new system that reacts to wind requires
no change to weather.

Three layers stay separate: **climate** is slow and feeds biome potential, **weather** is the current
state evolving over cells and storms, and **presentation** draws it. That separation is what lets a
strategy game make rain slow units deterministically while the cloud renderer runs at whatever
quality the budget allows.

The **wind field** is the clearest case of the substrate paying off: foliage, VFX, cloth, water,
clouds and audio were all specified to sample it and nothing wrote it. Now one producer composes
prevailing wind, storms, terrain influence and local volumes, and everything agrees.

Precipitation does not simulate drops — shelter is answered by a coarse occlusion representation, and
wetness and snow are accumulating, decaying fields consumed by materials through the runtime virtual
texture path. And **macro ecosystem state** evolves for regions nobody is looking at, so a burned
forest regrows toward its potential as a few field values, with procedural generation materialising
the detail when someone arrives.
