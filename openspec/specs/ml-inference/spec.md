# ml-inference Specification

## Purpose

Defines **CyberML**: running trained machine-learning models inside the engine.
...
That boundary is the important part. Inference is not bit-reproducible across backends, devices, or
driver versions, so model output cannot drive authoritative gameplay unless the session is pinned
to a configuration the model asset declares as verified reproducible — enforced at cook time, so a
mismatch surfaces as a build error rather than as an unexplained multiplayer desync months later.

CyberML is optional. `ai-system` may consume it through a graph node; it does not depend on it.
