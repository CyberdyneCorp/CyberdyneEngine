...
used only where explicitly required: the simulation clock, accumulated time, geodetic or
large-world coordinate helpers, and analysis tooling.

Large-world support SHALL be provided by **camera-relative rendering** — transforms are rebased
to the camera before being submitted — rather than by making the whole engine double precision.

#### Scenario: Distant object stays stable
- **WHEN** an object is 100 km from the world origin
- **THEN** camera-relative rebasing SHALL keep its rendered transform free of visible jitter with
  32-bit floats
