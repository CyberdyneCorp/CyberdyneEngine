# denoising Specification

## Purpose

Defines **CyberDenoiser**: the single framework that reconstructs a stable image from every
stochastic signal the renderer produces — indirect diffuse, indirect specular, ray-traced shadows,
ambient occlusion, and stochastic direct lighting.

It exists for the same reason the temporal framework does. Each of those signals needs temporal
accumulation, variance estimation, edge-aware spatial filtering, and history validation; specified
per effect, they get implemented per effect, and the engine acquires five subtly different blurs
that disagree at edges.

Two properties are specified rather than left to tuning. Filtering is **material and geometry
aware** — it uses the visibility buffer's instance and material identifiers, which give exact
boundaries instead of ones inferred from depth and normals. And each signal **declares what kind
of signal it is**: a specular lobe tolerates a filter width that a near-mirror reflection does
not, and a visibility term like shadows or ambient occlusion must be reconstructed as occlusion
rather than as colour, or contact detail is filtered away.

Denoiser quality is one of the levers the illumination budget can pull, and the whole thing can be
switched off to inspect the raw signal against a path-traced reference.
