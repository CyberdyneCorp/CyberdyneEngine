...
The governing idea is that **prefabs are an authoring and compilation system, not a runtime object
hierarchy**. Nesting, variants, and overrides are resolved at cook time into immutable
**entity templates** — archetype blocks with prepared component data — so spawning is a bulk copy
rather than a graph walk. Shipping builds carry no prefab link at all; development builds keep
provenance, which is what makes live prefab editing possible.

Three asset kinds are kept semantically distinct rather than collapsed into one container: a
**prefab** is a reusable object, a **scene** is a reusable spatial composition, and a **world** is
the top-level spatial universe (see `world-partition-and-streaming`).

Two decisions here are deliberate improvements on the engines this borrows from. **Exposed
parameters** give a prefab a declared public interface, so its internals can be refactored without
breaking every instance — arbitrary component overrides make every internal detail part of the
contract. And **overrides address stable identifiers rather than name paths**, so renaming an
entity, component, or field does not silently destroy a designer's work.
