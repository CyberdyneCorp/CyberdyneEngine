# input-and-actions Specification

## Purpose

Defines **CyberInput**: how device signals become semantic actions, and how actions become the
gameplay intent the simulation consumes.

The boundaries are the specification. The platform layer ends at normalised, **timestamped** device
events; this layer owns users, contexts, bindings, processors, triggers and actions; gameplay owns
commands. A gameplay system that reads a key code has crossed a line, and so has an input layer that
writes a transform.

**Input users** make local multiplayer structural rather than retrofitted: a user owns devices, a
context stack and a rebinding profile, so two people on one machine are two users and a disconnected
controller is a user awaiting a device, not a lost player. A **layered context stack** with priority
and consumption handles a modal over an inventory over a vehicle without anyone remembering what to
re-enable.

Two details matter more than they look. A mouse delta is displacement and a stick is a rate, so a
binding declares its interpretation and a delta is never scaled by frame time — the most common
input defect in shipped games and nearly invisible until someone changes frame rate. And fixed-tick
sampling resolves **timestamped accumulated events**, not whatever value was current when the tick
began, which is what makes a fast press survive and a replay faithful.

Accessibility is part of the model rather than a layer on top, which is a further reason raw device
access is not the gameplay path: transformations applied in the pipeline cannot be bypassed.
