# editor-agent-interface Specification

## Purpose

Defines how an **autonomous agent drives the editor**: reading project state, invoking editor
actions, manipulating objects, and observing the rendered result — over the Model Context Protocol.

The design decision that makes this small is that it is a **projection, not a second API**.
`editor-rust-application` already requires every user-invokable action to be a registered command,
already forbids an action reachable only through one widget, and already routes state-modifying
commands through the transaction system. Given that, the tools *are* the registry, the mutations
*are* transactions, and the refusals *are* the existing availability predicates. There is nothing to
enumerate and nothing to keep in sync — which is what stops an agent API becoming a second way to
mutate a project that undo only partly understands.

The half that matters most is observation. An agent that can place a light but cannot see the scene
is now blown out is doing data entry. So the agent sees the viewport through the engine's own
renderer, on the same path as the human's viewport, for the same reason the editor has no second
renderer: what the agent evaluates must be what the project will look like.

Attribution is specified and explicitly is **not** a security control. Scope and effect class govern
what an agent may reach; attribution answers the different question of who changed something and
what they were trying to do — which no engine's history currently answers at all.
