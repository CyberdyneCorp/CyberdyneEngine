...
`CY_NAVIGATION`, `CY_AUDIO`, `CY_UI`, `CY_NETWORKING`, `CY_XR`, `CY_PROFILING`,
`CY_RENDERER_VULKAN`, `CY_RENDERER_METAL`, `CY_RENDERER_D3D12`, `CY_SANITIZE`.
...
- **WHEN** rendering, audio, UI, XR, and the editor are disabled
...
- reflection registration code for engine types
...
Generated files SHALL be written to the build directory, except the Swift overlay, which SHALL be
committed so consumers need no generator.
...
per platform and configuration used when packaging a game, and the **tools** (cooker, packager,
shader compiler).

Artefact names SHALL encode platform, architecture, configuration, and version.
