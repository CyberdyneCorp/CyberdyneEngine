# project-and-plugins Specification

## Purpose

Defines what a project is made of and how the engine is extended: the project graph, modules and
their dependencies, plugins, extension points, and layered configuration.
...
**architectural layering is enforced by the build** — a runtime module may not depend on an editor
module, and a foundation module may not depend upward. That is enforced mechanically rather than by
review because dependency direction decays silently and is the most expensive architectural property
to recover once lost.

Extension happens only through declared, independently versioned **extension points**, which the
engine's own features use so that inadequacies are found internally first.

The binary plugin boundary is deliberately not new: it is the engine's **existing stable C ABI**.
Introducing a second versioned surface would double the compatibility burden for no gain. First-party
modules compiled together use native C++, where ABI stability across versions is unnecessary.

Two limits are stated rather than implied. A module may not unload while live instances of the types
it owns exist. And native code loaded into the process has the process's privileges, so extensions
are classified by **trust tier** rather than described as sandboxed.
