...
fixed-point path. And the engine has **no world partition capability** — replication cells are
networking-owned, with the integration contract specified so a future world partition can supply
them instead.
...
The engine SHALL define the integration contract by which cell membership drives **asset
preloading** and **world content streaming**, so that a client entering a cell can have its content
resident before entities arrive.

The engine currently has **no world partition capability**; replication cells are therefore
networking-owned, and the contract is specified so that a future world partition system can supply
the partition instead without reworking network identity or interest management.

#### Scenario: Cell entry drives preloading
- **WHEN** a client's viewpoint approaches a new cell
- **THEN** the cell-entry event SHALL be observable so asset streaming can preload its content
  before replicated entities are spawned
...
- **WHEN** a world partition capability is later introduced
- **THEN** replication cells SHALL be able to derive from it without changes to network identity,
  interest scoring, or the scheduler
