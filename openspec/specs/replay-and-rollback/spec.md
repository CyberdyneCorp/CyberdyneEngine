# replay-and-rollback Specification

## Purpose

Defines how a session is recorded, reconstructed, rewound and re-simulated.

**One command log** serves replay, rollback and lockstep, because the gameplay command stream is
already the single semantic representation of intent and a second one would drift. Raw device input
is deliberately not the replay format: it would bind a recording to a binding set, an accessibility
configuration and a frame rate, none of which are part of the simulation.

What genuinely cannot be reproduced is **recorded rather than eliminated**. A service response, an
inference result, a secure random value, the real date — each is captured as an external result and
consumed from the record during playback, never re-invoked. That is what makes replay-stable a
useful profile for games that are not bit-deterministic.

Three snapshot kinds share schema metadata and deliberately share no encoding: rollback optimises
restore latency, replay checkpoints optimise compact seeking, saves optimise surviving code change.
Using the save format for rollback is forbidden, for the same reason tagged and cooked serialization
were separated.

The **side-effect ledger** solves the failure everyone hits: tick 100 plays an explosion, a late
input rolls back to 98, tick 100 runs again. Effects carry the simulation point that produced them
and are suppressed on re-simulation — one mechanism instead of fixing it one effect at a time.
