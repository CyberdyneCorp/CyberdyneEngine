...
Defines **CyberTerrain**: the ground surface as a streamed hierarchical dataset rather than a giant
mesh or a giant heightmap.

The decision that shapes everything else is that **terrain is a geometry source**. Tiles are meshed
into virtual geometry clusters that enter the GPU scene and are culled, shaded, and shadowed like
anything else — closing the seam `virtual-geometry` reserved, rather than adding a parallel
renderer with its own level-of-detail scheme and shadow path.

The honest qualification is that the heightfield does not go away. Physics, navigation, and
gameplay queries all want a surface they can sample cheaply, so terrain keeps its source
representation and derives several consumer representations from it. Rendering is unified; what is
not rendering stays separate, deliberately.

Three representations are admitted — heightfield, mesh, and signed distance field — with only the
first required, so that no consumer is permitted to assume terrain is a heightmap and caves become
an implementation rather than a rewrite. Deformation is classified by what it costs: a footprint
must not trigger a navigation rebuild. Runtime change is a delta over immutable cooked data,
carried by the world's existing persistence overlay rather than a second save path.
