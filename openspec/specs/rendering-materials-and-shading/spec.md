...
The engine SHALL implement these shading models, sharing one light iteration loop:

| Model | Additional terms |
|---|---|
| `Lit` | The core BRDF |
| `Unlit` | Emission only; no light iteration |
| `ClearCoat` | A second GGX lobe on the geometric normal with its own roughness and IOR 1.5, attenuating the base layer by `1 − Fc` |
| `Anisotropic` | Anisotropic GGX with tangent-space `αx`/`αy` derived from roughness and anisotropy |
| `SubsurfaceScattering` | Screen-space diffusion plus a transmittance term; see `rendering-post-processing` |
| `Cloth` | Charlie sheen distribution with an Ashikhmin visibility term, plus optional subsurface colour |
| `Hair` | Marschner-style R / TT / TRT lobes approximated for real time |
| `Foliage` | Two-sided lighting with a translucency term driven by a thickness map |

Each model SHALL be selectable per material and resolved through a specialization constant.
...
A **material** SHALL consist of: a material definition (shading model, blend mode, cull mode,
depth state, parameter declarations, texture slots — see `shader-system`), a set of parameter
values, and a set of texture bindings.

Materials SHALL support **instancing**: a material instance references a parent material and
overrides a subset of parameters, sharing the parent's pipeline.
...
- **THEN** all SHALL use the same pipeline, and sorting SHALL group them together
...
Material parameters SHALL be packed into a **global material buffer**, with each material
occupying a range addressed by a material index carried in instance data.

Textures SHALL be referenced by index into bindless descriptor arrays where available, and by
per-material descriptor sets otherwise.
