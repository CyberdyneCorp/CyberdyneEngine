...
The RHI SHALL support both classic descriptor sets and, where available, **bindless** descriptor
arrays for textures, samplers, and buffers.

Bindless SHALL be the preferred path when the device supports the required capabilities, with a
non-bindless fallback the engine can select at runtime.

#### Scenario: Bindless material access
- **WHEN** the device supports descriptor indexing
- **THEN** materials SHALL reference textures by index into a global descriptor array, so a
  material change does not require rebinding descriptor sets

#### Scenario: Fallback path
- **WHEN** the device lacks the required bindless capabilities
- **THEN** the engine SHALL use per-material descriptor sets, with the renderer's structure
  unchanged
