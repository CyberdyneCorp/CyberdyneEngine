# procedural-content-generation Specification

## Purpose

Defines **CyberPCG**: deterministic, region-based procedural generation over typed spatial datasets.

It exists because procedural generation was already happening in four places — foliage placement
rules, the terrain modifier stack, water's spline networks, and the beginnings of road and settlement
tooling — each with its own determinism story, invalidation and caching. This is the generalisation,
and those become consumers of it rather than being replaced.

Three decisions carry the weight. **Output is the representation the consumer wants**: ten million
trees are a foliage population, not ten million spawn commands, and a procedural result is not an
entity by default. **Generated identity is stable**, derived from generator, region and a per-output
key rather than from iteration order — which is what lets an author's override and a player's felled
tree survive regeneration, and what makes an orphaned override reportable rather than silently lost.
And generation is **region-based with declared dependencies and sampling radii**, so moving five
hundred metres of road regenerates the terrain stamps it cuts and the foliage within its clearance
radius, and nothing else.

Because a region's generation is a derivation with declared inputs and an immutable output, it is
already a build node: caching and distribution across workers come from the build system rather than
from a mechanism of its own.

Explainability is a requirement rather than a debugging feature. Procedural content is where the
artefact and its cause are furthest apart, so the tooling must answer both *why is this tree here*
