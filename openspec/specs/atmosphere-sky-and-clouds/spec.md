# atmosphere-sky-and-clouds Specification

## Purpose

Defines the sky: a physically based participating atmosphere, celestial bodies and time of day,
volumetric clouds, and the aerial perspective that replaces tuned distance fog.

The atmosphere is parameterised by physical quantities rather than by a tint, so a thin dusty sky
follows from its scattering coefficients — which matters for a project whose planet changes as it is
terraformed. Sky evaluation uses **precomputed tables** updated incrementally as the sun moves,
rather than ray marching per pixel.

Clouds are **reconstructed, not stored**. What streams and replicates is a coverage and type map at
kilometres of resolution; density is rebuilt procedurally during ray marching, at reduced resolution
with temporal reconstruction whose history semantics account for clouds moving independently of the
camera. Cloud shadows are a coarse world-scale field rather than virtual shadow pages, whose design
assumes shadow detail correlates with screen pixels — a cloud shadow's footprint is kilometres wide.

The rule that keeps it honest: **the same environmental state drives every quality tier**. Lowering
cloud quality changes how the sky is drawn, never how wet the ground is.
