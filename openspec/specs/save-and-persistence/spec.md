...
Defines durable game state: what is saved, how it is encoded, and how it survives code change.

A save **is the world's persistence overlay** plus scoped fragments, not a second persistence model.
The world already defines *authored cells plus overlay equals current world* and names saves as one
of its consumers; this capability supplies the encoding, journalling, atomicity, incrementality and
migration it lacked. Two models would be two sources of truth about a destroyed bridge.

Two requirements decide whether it survives contact with a large world. **Dirty tracking** means an
autosave inspects what changed rather than walking ten million entities. **Cell-scoped persistence**
means a world of which ninety-five per cent is unloaded is saved without loading any of it — the
obvious implementation instead produces a save whose cost grows with world size rather than with
what the player did.

Everything is a **delta against authored content**: an unchanged entity contributes nothing, a
destroyed authored entity contributes a tombstone, a player-built structure contributes a template
and its overrides. Nothing derived is saved; caches, indexes and residency state are rebuilt.

And a save never destroys the previous one. Chunks are written, verified, and the manifest switched
atomically, with generations retained — because an interrupted save that leaves neither the old
state nor the new one is lost progress on a player's machine.
