...
Ragdolls SHALL be supported as a configuration of bodies and constraints driven from a skeleton,
with blending between animated and simulated poses.

#### Scenario: Ragdoll activation
- **WHEN** a character's ragdoll is activated
- **THEN** its bones SHALL be driven by physics from their current animated pose, with an
  optional blend period
