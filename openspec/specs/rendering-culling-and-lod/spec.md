...
Instances SHALL be tested by projecting their bounds and comparing against the appropriate HZB
mip. A **visibility hysteresis** SHALL keep recently visible instances visible for a configurable
number of frames to prevent flicker from reprojection error.

Instances MAY opt out with an `IgnoreOcclusion` flag. False occlusion SHALL be impossible for
correctness-critical cases: reprojection SHALL be conservative, and newly disoccluded regions
SHALL be treated as visible.
...
An instance MAY declare a chain of LOD levels, each with a mesh and a screen-space error or
coverage threshold.

LOD selection SHALL be computed from projected screen coverage, adjusted by a global LOD bias, a
per-instance bias, and the view's quality setting. Shadow and reflection views SHALL use their
own bias so lower LODs are used where detail is not visible.
