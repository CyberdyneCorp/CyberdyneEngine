...
Defines **CyberGraph**: shared graph infrastructure with domain-specific lowering, plus gameplay and
ability graph languages.

The honest boundary is the important part. Materials, VFX, animation, AI and camera rigs already
compile graphs, and they keep their own intermediate representations — a material's algebra, a
particle kernel, a pose evaluation and a behaviour program are different languages. What they share
is everything around the language: nodes and pins, typed connections, stable identity, serialization,
subgraphs, the canvas, diffing and debugging. Five capabilities had each built some of it.

Graphs are an **authoring language, never the runtime object model**. There is no interpreter instance
per entity; per-entity state compiles to generated component data, and a behaviour applying to many
entities lowers to a system iterating chunks. Typed pins make errors compile-time, and events rather
than tick is the default idiom, so the easy path is the efficient one.

Two capabilities are better here than they usually are elsewhere. A graph compiler can **audit
determinism** across a whole program in a way review of handwritten code cannot, and can enforce
capability restriction for untrusted graphs at compile time. And **semantic diff and three-way
merge** — stable identity, layout separated from semantics, deterministic text — solves the problem
that usually makes visual scripting painful for a team.
