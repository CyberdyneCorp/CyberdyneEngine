# virtual-texturing Specification

## Purpose

Defines **CyberTexture**: extremely large logical textures backed by only the pages a frame actually
needs.

Four residency models sit behind one public handle — resident, mip-streamed, virtual streamed, and
virtual runtime — and the choice is a cook decision rather than an API distinction. Mip streaming is
kept as a first-class model rather than a legacy path, because virtualising every texture would be a
regression for most of a project's content.

A guaranteed-resident **mip tail** means a surface is never missing, only blurry — the third time
this engine makes that decision, after virtual geometry's always-resident root and virtual shadows'
fallback chain. **GPU feedback** deduplicates and compacts requests before any CPU involvement, and
the division of labour with prediction is explicit: prediction covers latency, feedback establishes
accuracy.

**Runtime producers** are first-class rather than a terrain feature: terrain material composition,
decal accumulation, world state, and procedural surfaces all write pages through one mechanism, so
an expensive material graph is evaluated once per page instead of per pixel per frame.

The trap this specification is written to avoid is derivative reconstruction. Virtual sampling needs
texture-space derivatives, and under a visibility buffer shading is decoupled from rasterisation, so
they must be reconstructed analytically from the identified primitive. Getting it wrong shimmers or
blurs the whole frame and is easily misattributed to the texture system for months.
