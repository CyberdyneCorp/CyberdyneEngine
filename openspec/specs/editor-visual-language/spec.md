# editor-visual-language Specification

## Purpose

Defines the editor's **appearance and vocabulary**: what it looks like, what its colours mean, and
what it calls things.

Three capabilities divide the editor's front end. `editor-ui-ux` owns interaction — what the user
does and how the editor responds. `editor-viewport-and-gizmos` owns behaviour — picking, stable
manipulation, one transaction per drag. This one owns appearance: that the scene carries the
screen's colour and the chrome recedes, that gold means selection and X is red, that arrows
translate and boxes scale, that a node is not an Actor.

It exists as a specification rather than a style guide because the failure is cumulative. What
decides a tool's appearance is a long series of small local decisions — this header needs emphasis,
this state needs a colour, this plugin needs an icon — each reasonable alone. By the time an editor
looks inconsistent, the inconsistency is spread across a hundred files and fixing it is a rewrite of
the interface rather than a change to it.

The reference imagery in `docs/design/` is normative, and states **visual language** — hierarchy,
density, colour, chrome, composition — rather than feature completeness. Every constraint here is
implementable at M5 against whatever the renderer can produce then.
