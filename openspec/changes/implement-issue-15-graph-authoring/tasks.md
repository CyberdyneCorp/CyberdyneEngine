...
- [ ] 2.1b Author separately saved modules, typed parameters, interface bindings, renderer settings, and CPU/GPU targets through transactions and undo/redo.
  - [x] 2.1b.1 Resolve saved `.cyvfxmodule` assets at cook time in `cy::vfx-compiler` (#19): missing, unmapped, cyclic, stage-incompatible and mistyped modules fail with diagnostics naming the module and scoped to the referencing emitter; each composed module's content digest (transitively, not its name or path) is in the cook key.
  - [x] 2.1b.2 Declare modules as build-graph dependencies: the `vfx` producer discovers every module a system reaches, so editing one re-cooks exactly the systems that use it (#19).
  - [x] 2.1b.3 Save and reopen a module and a system referencing it through `vfx.module.*` / `vfx.document.*` over MCP, with undo and redo (#19).
  - [ ] 2.1b.4 Record module graph edits in the desktop panel as per-edit transactions.
