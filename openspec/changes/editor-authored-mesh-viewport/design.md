...
9. **Synchronize new world declarations before live edits.** Transactions name component and field numbers declared in the world file. An empty world has no declarations, so a newly imported FBX can create a node in the runtime without giving the renderer enough schema to find its mesh. On first connection and whenever the editor's document schema changes, send the current unsaved `.cyworld` text through one bounded `SyncWorld` message. The runtime parses it into a temporary world, checks document identity, resolves engine types, and replaces its in-memory world before subsequent transactions. Ordinary edits continue to use the journal's `Apply` stream.
