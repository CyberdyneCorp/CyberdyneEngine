...
| **M8** | Game systems | Gameplay framework, abilities, visual scripting, sequencing, animation, AI, navigation, VFX, UI, text, 2D, full audio, inference — across three rungs, M8.a, M8.b and M8.c | A playable vertical-slice game, measured and photographed, exercising the gameplay-facing capabilities this milestone took to Working. It SHALL NOT be read as exercising every one of them: `vfx-system` closed M8.c at **Seed** with its Working cell moved to M10, because the GPU compute dispatch its default path needs was not built |
...
**M8 is a split, and it is recorded as one.** `split-m8-authorable-and-systems` divided it into
**M8.a** — a scene a person builds by hand: primitives, import from inside the editor, a body on an
object, play mode — and **M8.b**, everything that lowers through one graph IR; `split-m8b-defer-spectacle`
then divided M8.b again, moving VFX, cinematics and inference into **M8.c**. Both applied the rule
this specification already carries: a milestone whose artefact cannot be reached without its own risk
spike succeeding, when some other coherent artefact could be reached without it, contains two. The
lettered rungs keep every reference to M9 through M11 valid, as M5.5's fractional number does, and
`tools/roadmap/record.py`'s `MILESTONES` is the machine-readable form of this list.
...
#### Scenario: An artefact's row describes what it exercised
- **WHEN** a milestone closes without a capability its row names reaching the tier the row implies
- **THEN** the row SHALL be corrected to say what the artefact exercised and to name the capability
  that did not reach it, rather than being left as a claim the record does not support
