...
- [ ] 1.1 **A node preview is produced by the runtime compiler and by nothing else.** The preview of
...
- [ ] 1.2 **The compiler reports which material inputs are bound to textures and which are
...
- [ ] 1.3 **Every lowering stage `cy_material compile` prints is reachable through the editor's front
      ...
      *onto the compiler* able to show every stage — so the check compares the stage list and the
      ...
      when they diverge
- [ ] 1.4 Prove both criteria can fail: rename or drop one lowering stage and watch the comparison go
