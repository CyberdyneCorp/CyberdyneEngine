...
- [ ] 3.1 Implement native shared-surface publishing and synchronized import behind the existing transport abstraction; verify producer/consumer ownership across a bounded ring.
- [ ] 3.2 Enable and test the Darwin runtime-control bridge; exercise pick identity, resize, disconnect, and restart across real processes.
- [ ] 3.3 Capture an engine frame in the macOS editor and record latency/frame pacing with the device and transport named.

## 4. Integration handoff

- [ ] 4.1 Run native conformance and relevant editor tests, update backend documentation, and report unresolved scope to the roadmap owner without editing their milestone status.
