...
8. Attach Swift `@Behaviour` classes to scene nodes through a `ScriptBehaviour.class` authoring field. The hosted runtime loads the newest built project module at Play entry, binds it to the Play ECS world through the existing generated C ABI, and calls fixed updates between physics and scene propagation. Script writes then reach the authored frame through the Play session's placement publication. Reject Play when a requested script has no built module or registered class; destroy behaviour instances before the Play world is released. Audio remains a separately tracked service.
