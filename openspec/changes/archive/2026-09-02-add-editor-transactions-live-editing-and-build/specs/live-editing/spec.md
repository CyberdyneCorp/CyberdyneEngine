...
| Mode | Purpose |
|---|---|
| `InEditor` | Fast iteration; a runtime world inside the editor process |
| `SeparateProcess` | Closer to shipping behaviour; isolates editor state from runtime behaviour |
| `RemoteDevice` | The game runs on a console, phone, tablet, or another machine |

All three SHALL be driven through the **same live bridge interface**. In-process operation SHALL be
an optimisation of transport, not a different architecture, so that remote play requires no separate
implementation.

Play mode SHALL support pause, single frame step, and single simulation tick step in every mode
...
- **THEN** live editing, inspection, and profiling SHALL work through the same interface used
  in-process
