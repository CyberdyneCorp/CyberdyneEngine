...
- `serialization-and-prefabs` — **the cooker emits the reference sites.** The flattening spike
  measured that activation is a bulk copy *plus* a bounded fixup — the key column and the reference
  slots — and that resolving which columns hold references by asking the registry per row costs
  4.7–5.2× more than a cooked site table. Without the table, activation degrades into the per-entity
  reflection walk cooking exists to eliminate. The requirement now says the table is cooked.
