...
The system SHALL adjust the screen-error threshold to hold a configured **GPU time budget** for
virtual geometry, raising the threshold under pressure and lowering it when headroom exists.

Adjustment SHALL be smooth and hysteretic: detail SHALL NOT visibly pump between frames.
...
The threshold SHALL be settable per view, so secondary views cost less.

A **pinned mode** SHALL disable adaptation for capture and deterministic testing.

#### Scenario: Budget is held
- **WHEN** measured virtual geometry GPU time exceeds its target
- **THEN** the threshold SHALL be raised until the budget is met, with the adjustment reported
