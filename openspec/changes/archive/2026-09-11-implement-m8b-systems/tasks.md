...
- [x] 2.1 **CyberGraph** — the shared authoring layer every consumer adopts: nodes, typed pins, typed
...
- [x] 2.2 **A shared pure-expression SSA core**, generalised from the material IR by the four named
...
- [x] 2.3 **The core's criterion is an anchor, not a port**: given the material op table and type
...
- [x] 3.1 Rules, session fragments, participants and teams
- [x] 3.2 Ownership, control and authority; capabilities, tags, phases
- [x] 3.3 Time domains, interaction, features, indexes, on M8.a's `PlaySession`

## 4. Abilities — `gameplay-abilities-and-effects` → W

- [x] 4.1 Compiled ability programs over section 2's IR
- [x] 4.2 Attributes and modifiers, effects and stacking, costs and cooldowns
- [x] 4.3 Targeting and the activation pipeline

## 5. Animation — `animation-and-skinning` → W

- [x] 5.1 Skeletons and bone LOD, clips and compression
- [x] 5.2 The animation graph compiled to a program, batched evaluation, layers and masks
- [x] 5.3 Root motion, IK, retargeting, the GPU pose world, pose sharing
- [x] 5.4 `rendering-geometry-and-resources`' "Skinning" requirement, which
...
- [x] 6.1 Navmesh over Recast, runtime updates, streaming, A* and funnel, hierarchical paths
- [x] 6.2 Flow fields, off-mesh links, avoidance, crowds
- [x] 6.3 Agents as entities; the unified graph with tree, utility and GOAP semantics
- [x] 6.4 Compiled behaviour programs, batched perception, knowledge, environment queries, smart
      objects, AI LOD
...
- [x] 7.3 Rig graphs compiled to programs, blending, framing, aim, shake, volumes, cuts

## 9. The interface — `ui-system` → W, `text-and-fonts` → C, `rendering-2d` → W

- [x] 9.1 Dedicated element storage, the retained tree, documents, declarative authoring, layout
- [x] 9.2 `.cyss`, data binding, input routing, the layer stack, the widget set
- [x] 9.3 GPU-driven rendering, accessibility checks, the interface's frame budget
- [x] 9.4 Shaping, BiDi, line breaking, layout objects, localisation
- [x] 9.5 Sprites, ordering, batching, tilemaps, 2D lights and shadows, the screen-space SDF

## 10. Audio — `audio` → C

- [x] 10.1 Steam Audio behind the engine's own interface, declared in `deps/manifest.toml` and
      `THIRD_PARTY.md` with a licence identifier and a justification
- [x] 10.2 Acoustic geometry, importance tiers, voice virtualisation, effects, interactive audio
...
- [x] 12.1 A level, characters that animate and think, abilities with effects, a heads-up
...
- [x] 12.2 8,000 agents and 100 concurrent effects hold their declared budgets
- [x] 12.3 Ability activation and animation evaluation are deterministic under the simulation's
      declared profile
- [x] 12.4 Runs from a single recipe; a recorded gap exits non-zero; the headline figure reproduces
- [x] 12.5 Commit a screenshot

## 13. Records and gates

- [x] 13.1 Write `tools/roadmap/milestones/m8b.toml`; declare `milestone-m8b` in `gates.toml` and
      raise `selftest.MINIMUM_CRITERIA`
- [x] 13.2 An `m9-open` criterion using the double-star glob form
- [ ] 13.3 Update `status.yaml`, `capability-matrix.md`, `ROADMAP.md` and `dependencies.md`, and run
      the plan-consistency checks over them
- [x] 13.4 Move `ci.yml`'s milestone job to `m8b` in the same commit that flips the gate green
- [x] 13.5 Open the M9 change

## 14. The gate

- [x] 14.1 Full audit, per `delivery-roadmap`
- [x] 14.2 Clean build of every profile from empty; `test-all` in each
- [x] 14.3 Every gate by hand; the M8.b ledger run once
- [x] 14.4 Every criterion actually executes something
...
- [x] 14.6 Records verified against what the code supports, not what the plan claimed — and the
...
Thirteen tasks are unchecked. The ledger passes — 217 criteria, exit zero — and the slice plays.

**The spike refuted the plan's premise and the milestone was rebuilt on the refutation.** One IR
cannot serve seven consumers: five escape hatches against a budget of two. What shipped is one
authoring layer, one shared expression core and a lowering each, and section 2's tasks read against
the plan that no longer applies.

**Two capabilities were demoted rather than claimed.** `audio` does not reach Complete because
`-D CY_AUDIO_STEAM_AUDIO=ON` cannot be configured at all — upstream wants PFFFT, IPP and FFTS as
pre-installed packages the manifest does not provide — and `SteamAudioBackend::simulate` returns
`NotImplemented`. `text-and-fonts` does not either: the engine can lay out Arabic and cannot load a
font to draw it with, because HarfBuzz, ICU and FreeType are each deferred by name. The algorithms
are real and are Working; the integrations are not.

**The renderer was assembled and stopped one layer short.** `FrameAssembly` hands each pass's record
callback to its caller and the slice supplied none, so the frame was real and nothing recorded into
it. That is why this milestone's artefact picture is drawn rather than captured, and it is what M8.c
was given as section 1b before it started.

**And the picture took its silhouettes from the sample's own table** rather than the mesh handle the
frame resolved — it would have drawn a defect correctly. Fixed, and the fix became a check that goes
red when the resolver breaks.
