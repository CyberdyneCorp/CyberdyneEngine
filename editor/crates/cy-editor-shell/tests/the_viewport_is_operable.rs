    ...
    let mut output = ctx.run_ui(raw.clone(), |ui| {
        egui::CentralPanel::default().show(ui, |ui| {
            ...
            let mut panels = Panels {
                ...
                source_control: &mut harness.source_control,
                asset_browser: &mut harness.asset_browser,
                source_workspace: &mut harness.source_workspace,
                agent: None,
                diff: &mut harness.diff,
                merge: &mut harness.merge,
