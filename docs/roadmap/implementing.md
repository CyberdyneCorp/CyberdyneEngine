...
**M11 · Reach — scoped, and it is six rungs.** `openspec/changes/implement-m11-reach/` asked the
...
| **M11.d** · Desktop | `implement-m11d-desktop/` | the RHI interface settled on Vulkan and null, a native `Platform` and `DisplayServer`, the core rows the port audits, the gate set | `samples/11-ship` on desktop |
| **M11.d.5** · Backends | `implement-m11d5-backends/` | Metal native and D3D12 from nothing — **inserted by M11.d's spike**, because neither compiles on the Linux host this project works on | **one scene, three backends, the same picture** |
...
[risks entry 12](risks.md). **M11.d.5 is the exception that proves the mechanism works**: it exists
*because* a spike ran, and it inherits M11.d's answer rather than owning a spike of its own. The seven inherited gaps are re-pointed rather
